...
def visualize_performance(y_K, pred_K, R2_K, acc_K, y_mu, pred_mu, R2_mu, acc_mu, rock, cover_interval, overlap,
                          excluded_subcube, save_dir):
...
def load_graph_data(data_dir, avg_scheme, batch_size, val_percentage):
    """
    Function for loading graph data, modified to return the test loader
    """
    ...
    # Here we assign valid_set to be the test dataset (for consistency with prepare_train_dataset)
    test_set = valid_set
...
def evaluate_model(model, loader, device, rock, cover_interval, overlap, excluded_subcube, save_dir):
    ...
    visualize_performance(y_whole_K, K_pred_whole, R2_K, acc_K_total, y_whole_mu, mu_pred_whole, R2_mu, acc_mu_total,
                          rock, cover_interval, overlap, excluded_subcube, save_dir)
    return R2_K, R2_mu

#====================================================================================================================================
def visualize_train_performance(y_K, pred_K, R2_K, acc_K, y_mu, pred_mu, R2_mu, acc_mu, rock, cover_interval, overlap,
                          excluded_subcube, save_dir):
    plt.rcParams['font.size'] = 5
    # Plotting for K
    fig, ax = plt.subplots(dpi=600, figsize=(5, 5))
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/Train_{rock}_excluded_subcube_{excluded_subcube}_R2_K.png')
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig(f'{save_dir}/Train_{rock}_excluded_subcube_{excluded_subcube}_R2_mu.png')

    print('Accuracy for mu', np.mean(acc_mu))

def evaluate_train_model(model, loader, device, rock, cover_interval, overlap, excluded_subcube, save_dir):
    # Evaluate the model on the test set
    model.eval()
    ...
    # Return values if needed later
    visualize_train_performance(y_whole_K, K_pred_whole, R2_K, acc_K_total, y_whole_mu, mu_pred_whole, R2_mu, acc_mu_total,
                          rock, cover_interval, overlap, excluded_subcube, save_dir)
    return R2_K, R2_mu